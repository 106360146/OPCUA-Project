...
class XmlImporter(object):
    ...
    def import_xml(self, xmlpath=None, xmlstring=None):
        ...
        if len(self.refs) != 0:
            self.logger.info("The following references could not be imported and are probaly broken: %s", self.refs) 
    ...
    def make_objects(self, node_datas):
        ...
        for ndata in node_datas:
            ...
            if ndata.parent:
                ...
                self.logger.debug("succes %s", ndata.parent)
            if ndata.parentlink:
    ...
    def _migrate_ns(self, nodeid):
        ...
        must be converted to a already existing namespace id based on the files_get_node
        namespace uri
    ...
    def _get_node(self, obj):
        ...
        self.logger.info("Importing xml node (%s, %s) as (%s %s) parent %s", obj.browsename, obj.nodeid, node.BrowseName, node.RequestedNewNodeId, obj.parent)
        node.NodeClass = getattr(ua.NodeClass, obj.nodetype[2:])
        self.logger.debug(f"node.ParentNodeId: {obj.parent}")
